...
9. As a founder, I want a "Verify key" button in Settings that sends a minimal test request to my LLM provider, so that I can confirm the key works before I try to use `/ask`.
...
17. As a founder, I want previous queries in the session shown as a history list, so that I can revisit or re-run earlier questions without retyping them.
...
22. As a founder, I want the displayed SQL to be exactly the SQL that ran — the post-validation, post-LIMIT-injection version — so that what I read matches what executed.
...
52. As a founder, I want pinned answers to be stored as saved queries (the SQL + display settings) in a `pinned_queries` table, so that the chart re-runs against live data on every dashboard load rather than showing a frozen snapshot.
...
Extended to load `pinned_queries` from the DB and render each as a chart card below the fixed metrics. Each card runs its saved SQL via `/api/ask` (or a dedicated `/api/pinned-query/[id]` route) and shows the same result + trend + row count as the `/ask` page.
...
- `signups_v` — alias-resolved, first_seen-based signup counts; encodes the correct join so the LLM doesn't re-derive it
- `active_users_v` — identified users with ≥1 event in a sliding window
- `company_activity_v` — company domains ranked by event count, personal domains pre-excluded
